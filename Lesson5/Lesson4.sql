...
select P.Name, Student.StudentNumber from Student
inner join dbo.People as P on P.Id = Student.PersonId


select People.Name, Student.StudentNumber from Student
right join dbo.People on People.Id = Student.PersonId
where Student.StudentNumber is not null

-- order by

select p.Name, sp.Point, S.SubjectName from Student
...
order by sp.Point desc

-- group by

select S.SubjectName, AVG(SP.Point) as AvgPoint from dbo.Student
inner join dbo.StudentPoint SP on dbo.Student.Id = SP.StudentId
inner join dbo.Subject S on S.Id = SP.SubjectId
group by S.SubjectName

select * from dbo.StudentPoint

insert into StudentPoint (StudentId, SubjectId, Point) values (1, 1, 5)
insert into StudentPoint (StudentId, SubjectId, Point) values (1, 2, 7)
insert into StudentPoint (StudentId, SubjectId, Point) values (1, 3, 9)

insert into StudentPoint (StudentId, SubjectId, Point) values (2, 1, 2)
insert into StudentPoint (StudentId, SubjectId, Point) values (2, 2, 2)
insert into StudentPoint (StudentId, SubjectId, Point) values (2, 3, 4)

insert into StudentPoint (StudentId, SubjectId, Point) values (3, 1, 12)
insert into StudentPoint (StudentId, SubjectId, Point) values (3, 2, 12)
insert into StudentPoint (StudentId, SubjectId, Point) values (3, 3, 12)


-- AVG point off all students
